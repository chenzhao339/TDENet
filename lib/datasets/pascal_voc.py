...
class pascal_voc(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        ...
        self._classes = ['__background__'] + cfg.TRAIN.ALLCLASSES
    ...
    def _do_python_eval(self, output_dir='output', **kwargs):
        ...
        print('Mean Base AP = {:.4f}'.format(np.mean(aps)))
        print('Mean Novel AP = {:.4f}'.format(np.mean(aps1)))

        checking2 = 'Mean Base AP = '+ str(round(np.mean(aps), 4)) +'\n'
        checking3 = 'Mean Novel AP = '+ str(round(np.mean(aps1), 4)) +'\n'
        fh = codecs.open('/home/ivp/paper1改/TDENet/test.txt', "a+", "UTF-8")
        fh.write(checking2)
        fh.write(checking3)
        fh.close()
